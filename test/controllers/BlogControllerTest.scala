    ...
    "should display the 'Graphics Notes' blog post" in {
      ...
      bodyText.replaceAll(" +", "") mustBe
        """<!DOCTYPE html>
          ...
          |            <li>“Clip space” is what GLSL understands coordinates to be. It’s always on a plane of -1 to 1 for both mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers and y axis regardless of height and width. To pass in “normal” vertices you have to convert them to clip space.</li>
          ...
          |        <ul>
          |            <li>Create a new rotated position: rotatedPosition = vec2(a_position.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers * u_rotation.y + a_position.y * u_rotation.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, a_position.y * u_rotation.y - a_position.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers * u_rotation.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers)</li>
          ...
          |            <li>Multiply the Y by Y on the circle and subtract mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers times the mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers on the circle for the final Y.</li>
    ...
    "should display the 'Building a 3D Renderer From Scratch Part 1: TGA Images' blog post" in {
      ...
      bodyText.replaceAll(" +", "") mustBe
        """<!DOCTYPE html>
          ...
          |    depth. The image specification which includes the mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers and y origins, the width and height of the image, how many bits per pixel, and an optional image descriptor which we can ignore. The body of the file will simply
          |    follow this format:
          |</p>
          |    <pre><code>
          |        typedef struct
          ...
          |</p>
          |<pre><code>
          |        TgaColor get(int mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, int y);
          |        bool set(int mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, int y, TgaColor c);
          |
          |        TgaColor TgaImage::get(int mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, int y)
          |        {
          |            if (!data || mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers < 0 || y < 0 || mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers >= width || y >= height)
          |            {
          |                return TgaColor();
          |            }
          |            return TgaColor(data + (mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers + y * width) * bytesPerPixel, bytesPerPixel);
          |        }
          |
          |        bool TgaImage::set(int mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, int y, TgaColor c)
          |        {
          |            if (!data || mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers < 0 || y < 0 || mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers >= width || y >= height)
          |            {
          |                return false;
          |            }
          |            memcpy(data + (mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers + y * width) * bytesPerPixel, c.raw, bytesPerPixel);
          |            return true;
          |        }
          |</code></pre>
          |<p>
          ...
          |                flipHorizontally();
          |            }
          |            std::cerr << width << "mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers" << height << "/" << bytesPerPixel * 8 << "\n";
          |            in.close();
          |            return true;
          |        }
          |</code></pre>
    ...
    "should display the 'Building a 3D Renderer From Scratch Part 2: Drawing Lines' blog post" in {
      ...
      bodyText.replaceAll(" +", "") mustBe
        """<!DOCTYPE html>
          ...
          |</p>
          |<p>
          |    &emsp;&emsp;Where mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers and y represent any given point along the line, m represents the slope of the line (or how steep the line is), and y<sub>0</sub>
          |    represents the value of y when mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers = 0 (otherwise referred to as the y-intercept because that's the point at which the line intercepts the
          ...
          |</p>
          |<pre><code>
          |        y = (m * mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers) + b
          |        y - b = (m * mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers) // subtract b from both sides
          |        -b = (m * mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers) - y // subtract y from both sides
          |        -b = -y + (m * mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers) // flip right side around
          |        b = y - (m * mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers) // negate both sides
          |</code></pre>
          |<p>
          ...
          |</p>
          |<pre><code>
          |        for (int mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers = x0; mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers <= x1; mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers++)
          |        {
          |            int y = (m * mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers) + b;
          |            image.set(mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, y, color);
          |        }
          |</code></pre>
          |<p>
          ...
          |    Cool, me too. Firstly, the choice of constant for our iterator matters greatly. If we change <code>for (int mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers = x0; mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers <= x1; mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers++)</code> to be
          |    <code>for (int mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers = x0; mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers <= x1; mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers += 10)</code> for example, changing the constant from 1 to 10, we get:
          |</p>
          |<p>
          ...
          |    actually a huge deal. We can still remedy this though by always getting the minimum mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers and looping to the maximum mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers.
          |</p>
          |<pre><code>
          |        int minX = std::min(x0, x1);
          |        int maxX = std::max(x0, x1);
          |        for (int mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers = minX; mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers <= maxX; mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers++)
          |        {
          |            int y = (m * mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers) + b;
          |            image.set(mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, y, color);
          |        }
          |</code></pre>
          |<p>
          |    &emsp;&emsp;In the same vein, what about straight up and down lines where the mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers values for both points are equal? Right now those types of lines
          ...
          |    we can rearrange our y = mx + y<sub>0</sub> formula from earlier to be mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers = (y - y<sub>0</sub>) / m. You also might notice after drawing a few different lines that steep lines tend
          ...
          |                {
          |                    int mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers = (y - b) / m;
          |                    image.set(mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, y, color);
          |                }
          |            }
          |            else
          ...
          |        {
          |            char type = "v";
          |            float mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, y, z;
          |        } GEOMETRIC_VERTEX;
          |
          |        typedef struct
          |        {
          ...
          |                struct
          |                {
          |                    t mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, y, z;
          |                };
          |                struct
          |                {
          |                    t iVert, iuv, iNorm;
          ...
          |            };
          |
          |            vec3() : mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers(0), y(0), z(0)
          |            {
          |            }
          |
          |            vec3(t mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, t y, t z) : mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers(mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers), y(y), z(z)
          |            {
          |            }
          |
          |            inline vec3&lt;t&gt; operator^(const vec3&lt;t&gt; &v) const
          |            {
          |                return vec3&lt;t&gt;(y * v.z - z * v.y, z * v.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers - mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers * v.z, mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers * v.y - y * v.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers);
          |            }
          |
          |            inline vec3&lt;t&gt; operator+(const vec3&lt;t&gt; &v) const
          |            {
          |                return vec3&lt;t&gt;(mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers + v.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers, y + v.y, z + v.z);
          |            }
          |
          |            inline vec3&lt;t&gt; operator-(const vec3&lt;t&gt; &v) const
          |            {
          ...
          |            {
          |                return vec3&lt;t&gt;(mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers * f, y * f, z * f);
          |            }
          |
          |            inline t operator*(const vec3&lt;t&gt; &v) const
          |            {
          |                return mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers * v.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers + y * v.y + z * v.z;
          |            }
          |
          |            float norm() const
          |            {
          |                return std::sqrt(mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers * mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers + y * y + z * z);
          |            }
          |
          ...
          |        {
          |            s &lt;&lt; "(" &lt;&lt; v.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers &lt;&lt; ", " &lt;&lt; v.y &lt;&lt; ", " &lt;&lt; v.z &lt;&lt; ")\n";
          |            return s;
          |        }
          |</code></pre>
          |<p>
          ...
          |    need multiple points? This vec3 struct only stores the mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers y and z coordinates of one point." Right you are, imaginary reader! When a vector is defined as one point it can be assumed that the direction is moving
          ...
          |                    int x0 = (v0.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers + 1) * width / 2;
          |                    int y0 = (v0.y + 1) * height / 2;
          |                    int x1 = (v1.mapTotalSeasonNumberToArrayOfIndividualSeasonNumbers + 1) * width / 2;
